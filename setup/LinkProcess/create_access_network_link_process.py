...
load_dotenv()
json_file_path = os.getenv("PROJECT_PATH") + "/LinkProcess/AccessNetwork"
...
while i <= EDGE_SERVER_NUM:
    full_path = json_file_path + "/access_network_link_process_" + str(i) + ".json"
...
with open(psnode_config_file) as file:
    ...
    for psnode in psnodes_array:
        ...
        while i <= EDGE_SERVER_NUM:
            if str(i) == server_num:
                link_socket_addr = LINK_SOCKET_ADDRESS_ROOT + "access-network_" + vnode_id + "_" + pnode_id + ".sock"
                full_path = json_file_path + "/access_network_link_process_" + server_num + ".json"
